...
class PriorityQueue(Container[T]):
    ...
    def add(self, item: T) -> None:
        ...
        for element in self._queue[::-1]:
            if item < element:
                # if the item being added is less it's added after the object
                # hence the +1
                self._queue.insert(self._queue.index(element) + 1, item)
                return
        # if item has a priority lower or equal to everything in queue
        # add it to the back
