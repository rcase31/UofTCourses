...
class Game:
    ...
    def __init__(self, max_depth: int,
        ...
        for player_number in range(num_human):
            ...
            new_player = HumanPlayer(self.renderer, player_number,
                                     goal(random_colour))
            self.players.append(new_player)
            # displays this player's goal and draws it on the board
            self.renderer.display_goal(new_player)
            self.renderer.draw(self.board, player_number)

        # Creating all random players
        for _ in range(random_players):
            player_id = len(self.players)
            random_colour = random.choice(COLOUR_LIST)
            new_player = RandomPlayer(self.renderer, player_id,
                                      goal(random_colour))
            self.players.append(new_player)
            # displays this player's goal and draws it on the board
            self.renderer.display_goal(new_player)
        ...
        for player_difficulty in smart_players:
            player_id = len(self.players)
            random_colour = random.choice(COLOUR_LIST)
            new_player = SmartPlayer(self.renderer, player_id,
                                     goal(random_colour), player_difficulty)
            self.players.append(new_player)
            # displays this player's goal and draws it on the board
            self.renderer.display_goal(new_player)
            self.renderer.draw(self.board, player_id)
...
def random_game() -> None:
    """ Run a game with 5 random players and 1 smart
    """
    game = Game(5, 0, 5, [5])
    game.run_game(5)
